 ...
 * Here, blocks are just vectors with integer coordinates; however the type of their coordinates is double.
 */
...
public class RayIterator implements Iterator<Vector> {
    ...
    private Vector[] blockQueue = new Vector[3];
    ...
    public RayIterator(Vector start, Vector direction, int maxDistance) {
        ...
        Vector startBlock = start.clone();
        
        startBlock.floorVector();
        ...
        if (!startBlockFound) {
            throw new IllegalStateException("Start block missed in RayIterator");
        }
    ...
    private boolean blockEquals(Vector a, Vector b) {
    ...
    private double getXPosition(Vector direction, Vector position, Vector block) {
        return getPosition(direction.getX(), position.getX(), block.getX());
    }

    private double getYPosition(Vector direction, Vector position, Vector block) {
        return getPosition(direction.getY(), position.getY(), block.getY());
    }

    private double getZPosition(Vector direction, Vector position, Vector block) {
    ...
    public RayIterator(Vector start, Vector direction) {
