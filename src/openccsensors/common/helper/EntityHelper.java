...
public class EntityHelper {
	...
	public static HashMap<String, Entity> getEntities(World world, int x,
			int y, int z, double radius) {
		HashMap<String, Entity> map = new HashMap<String, Entity>();
		Vec3 sensorPos = Vec3.createVectorHelper(x, y, z);

		int dChunk = (int) Math.ceil(radius / 16.0F); // calculate the maximum
														// distance in chunks to
														// search in

		for (int dx = -dChunk; dx <= dChunk; dx++) {
			for (int dz = -dChunk; dz <= dChunk; dz++) {
				Chunk chunk = world.getChunkFromBlockCoords(x + 16 * dx, z + 16
						* dz);
				for (List<Entity> subchunk : chunk.entityLists) {
					for (Entity entity : subchunk) {
						...
						Vec3 livingPos = Vec3.createVectorHelper(
								...
								entityLiving.posZ + 0.5);
						if (sensorPos.distanceTo(livingPos) <= radius) {
							// Only append the id when it's not a player.
							// EntityPlayer already appends the id:
							String targetName = (entityLiving instanceof EntityPlayer) ? entityLiving
									.getEntityName() : entityLiving
									.getEntityName() + entityLiving.entityId;
	...
	public static HashMap livingToMap(EntityLiving living) {
		...
		map.put("HeldItem",
				InventoryHelper.itemstackToMap(living.getHeldItem()));

		HashMap armour = new HashMap();
		armour.put("Boots",
				InventoryHelper.itemstackToMap(living.getCurrentItemOrArmor(1)));
		armour.put("Leggings",
				InventoryHelper.itemstackToMap(living.getCurrentItemOrArmor(2)));
		armour.put("Chestplate",
				InventoryHelper.itemstackToMap(living.getCurrentItemOrArmor(3)));
		armour.put("Helmet",
				InventoryHelper.itemstackToMap(living.getCurrentItemOrArmor(4)));
